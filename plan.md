...
- Ship a personally signed application with adaptive light/dark styling, multi-playlist management, search, groups, favorites, recents, saved live-buffer recordings, now/next EPG, fullscreen, Picture in Picture, and AirPlay. Development builds may use an explicitly located Homebrew FFmpeg; distributable sandboxed builds require a separately built, relocatable, signed auxiliary executable.
...
- Add a **Save Buffer** checkbox beside the live DVR controls with a compact, persistent quality menu. **Original Video** preserves source resolution, codec, and HDR while normalizing audio to AAC; **Compatible 1080p** saves the H.264/AAC AirPlay rendition. Enabling it retains the selected current seekable window and every subsequently completed segment without a duration cap; disabling it or switching channels finalizes the capture. Clearly show active duration, finalization, failures, and the fact that disk usage grows until recording stops.
- Add Recordings under Library with programme-first titles, channel/date/duration metadata, locally generated video thumbnails, search, native seekable playback, Show in Finder, and confirmed deletion. Preserve a recording independently of later playlist refresh or source removal.
...
- Store non-secret source, channel, programme, favorite, recent, recording, and refresh metadata with SwiftData. Derive stable channel identity from source UUID plus normalized TVG ID, name, and group.
- Store each finalized recording as a self-contained package under Application Support containing a native MPEG-TS media file. Maintain original-video/AAC and compatible H.264/AAC rolling outputs from one FFmpeg input, append completed segments from the user's chosen quality directly to the recording, adopt the complete available window when Save Buffer starts, finalize in constant time, and extract a local JPEG thumbnail afterward so channel switching is not delayed. Safely migrate packages from the earlier local-HLS format on first playback.
...
- Accept user-configured HTTP and HTTPS playlist, EPG, and media URLs because many IPTV providers expose only HTTP endpoints. Fetch source documents through the bounded, redirect-limited AsyncHTTPClient transport so arbitrary HTTP hosts work without disabling ATS app-wide; continue preferring HTTPS and warn that HTTP credentials and content are visible on the network. When preparing a distributable build, enable App Sandbox plus network client/server entitlements and embed the transcoder inside the app's signed code graph.
...
- Produce a parallel source-resolution video/AAC rolling rendition alongside the AirPlay-compatible rendition from the same FFmpeg input. Use the selected rendition for Save Buffer; never start a second provider request, never persist provider or relay URLs in recording metadata, and validate all package and media paths before creation, playback, or deletion.
...
- Test compatible and source-video recording capture from their existing rolling windows, exact adopted-duration and quality reporting, continued native-file retention, legacy HLS migration, cleanup boundaries, package-path traversal rejection, SwiftData metadata round trips, thumbnail fallback, and persistence after the live transcode directory is removed.
...
- Scheduled recordings, cross-channel recording, catch-up beyond the locally retained window, downloads/exports, a timeline guide grid, reminders, local-file import, iCloud sync, and App Store submission remain out of scope.
- Save Buffer intentionally has no application-level duration limit. It runs only for the currently tuned relayed channel, ends on channel change or explicit disable, and remains bounded by available disk space and filesystem limits. Original Video is the default; the choice is available beside Save Buffer and in Settings, and cannot change mid-recording.
- AirPlay remains dependent on the receiver reaching the Mac and supporting the generated H.264/AAC rendition. The receiver-facing compatibility rendition is capped at 1080p; original-resolution video is retained only in local recordings, and playback still depends on the Mac supporting the source video codec.
