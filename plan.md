...
- Use Apple's native media stack (`AVPlayer`, `AVPlayerView`, and `AVRoutePickerView`) for playback and rendering. When a secure AirPlay relay is configured, permit an isolated FFmpeg process to copy compatible H.264 video and convert receiver-incompatible audio to AAC before AVPlayer/AirPlay consumes it; there is no alternate in-app video decoder.
...
- Show channel logo, name, group, live state, current/next programme, and favorite state. Selecting a channel immediately replaces the single active player item.
...
- For HLS (`.m3u8`), create an AirPlay-compatible rolling rendition when FFmpeg is available: copy the first H.264 video stream without re-encoding, convert the first audio stream to 48 kHz stereo AAC, retain enough generated segments to survive receiver handoff, and serve the generated playlist and segments through the same trusted HTTPS listener.
...
- Test transcoder executable discovery, real-time input pacing, command construction, startup/timeout/termination cleanup, playlist normalization, gzip negotiation, strict generated-file routing, byte ranges, and traversal rejection. Manually verify an H.264/MP2 source is exposed to AVPlayer and Apple TV as H.264/AAC with a live edge aligned to wall time.
...
- AirPlay remains dependent on the receiver reaching the Mac and supporting the generated H.264/AAC rendition. Video transcoding and arbitrary video codec conversion remain out of scope.
