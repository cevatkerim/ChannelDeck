...
For compatible streams, the relay copies H.264 video without re-encoding and converts the first audio track to 48 kHz stereo AAC in a rolling local rendition. If it detects HEVC or another non-H.264 video codec, it automatically restarts with Apple's VideoToolbox H.264 encoder and caps the receiver rendition at 1080p; this allows common 4K/50 HEVC IPTV services to play without software-encoding a 4K stream. Hardware-converted streams become playable after three complete segments while the rolling buffer continues to grow; copied streams retain a six-segment handoff buffer. AVPlayer remains the app's native renderer, and FFmpeg is used only to prepare receiver-compatible relay media. ChannelDeck looks for a bundled auxiliary executable first, followed by `/opt/homebrew/bin/ffmpeg` and `/usr/local/bin/ffmpeg`. Provider URLs are never passed to the subprocess—the transcoder receives either an opaque local relay URL or source bytes through standard input.

Continuous raw MPEG-TS sources are streamed through a bounded pipe into the same H.264/AAC relay without placing their URLs in subprocess arguments. The Mac must remain awake on the same LAN as the AirPlay receiver. Some routers' DNS-rebinding protection may block public hostnames that resolve to private addresses; allow the generated hostname if necessary.
